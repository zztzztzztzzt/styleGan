...
def grid_loss(pred_size, target_size):
    # 计算目标尺寸的损失
    loss_size = torch.abs(pred_size - target_size)
    
    # 求和或平均损失值
    grid_loss = torch.mean(loss_size)  # 可根据具体需求选择平均值或求和
...
def Landmark_loss(landmark_p, landmark_label, w=0.06, epsilon=0.01):
    x = landmark_label - landmark_p#[68*2] 我们是图像啊 点和点
