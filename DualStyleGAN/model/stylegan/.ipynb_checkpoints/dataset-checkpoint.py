...
class FramesDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.is_train and self.id_sampling:
            ...
            path_all = glob.glob(os.path.join(self.root_dir, name))
            rand_idx = True
            while (rand_idx):
                random_number = random.randint(0, len(self.videos)-1)
                name_S = self.videos[random_number]
                id_S = name_S.split("#")[0]
                id = name.split("#")[0]
                if id_S != id:
                    rand_idx = False

            temp = np.random.choice(path_all, replace=True, size=3)
            temp_s = np.random.choice(glob.glob(os.path.join(self.root_dir, name_S)), replace=True, size=1)
            path = temp[0]
            path2 = temp[1]
            path3 = temp[2]
            path4 = temp_s[0]
