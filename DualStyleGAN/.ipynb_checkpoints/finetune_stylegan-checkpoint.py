...
from model.stylegan.dataset import Dataset,FramesDataset
...
def train(args, loader, generator, discriminator, g_optim, d_optim, g_ema,id_loss, device):
    ...
    for idx in pbar:
        ...
        if pt2.numel() > 0 and pt1.numel()> 0:  # 检查pt2是否不为空
            pt_D = torch.cat([pt1, pt2], dim=0)
        else:
            continue

        real_img_D = torch.cat([real_img_D, real_img_D_noid], dim=0)#前面的是相同id后面是不同id
        real_img_S = torch.cat([real_img_S, real_img_S_noid], dim=0)
        real_img_D_img_pt = torch.cat([real_img_D_img_pt, real_img_D_img_pt_noid], dim=0)
...
if __name__ == "__main__":
    ...
    dataset = FramesDataset(root_dir=r"/home/featurize/DualStyleGAN/data/vox1_1s/vox1_s/train")
